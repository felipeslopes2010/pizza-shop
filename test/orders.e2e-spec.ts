...
test('filter by order id', async ({ page }) => {
    ...
    await expect(page.getByRole('cell', { name: 'order-1', exact: true })).not.toBeVisible();
});
...
test('filter by customer name', async ({ page }) => {
    ...
    await expect(page.getByRole('cell', { name: 'Customer 11', exact: true })).toBeVisible();
    await expect(page.getByRole('cell', { name: 'Customer 1', exact: true })).not.toBeVisible();
});
